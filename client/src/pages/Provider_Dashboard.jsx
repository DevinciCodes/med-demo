...
export default function ProviderDashboard() {
  ...
  const { signOut: ctxSignOut, user } = useAuth() || {}; // ⬅️ get user from context
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const uid = user?.uid;
        if (!uid) {
          // If auth hasn't hydrated yet, just stop quietly.
  ...
  // ===== medication dropdown options (ALL & OTC) =====
  ...
  const [medSource, setMedSource] = useState("all"); // 'all' | 'otc'
  ...
  // Combined meds list for add-med section (all + otc, deduped)
  ...
  const providerConflicts = useMemo(
    () => findConflicts(medications),
    [medications]
  );
  ...
  const handleCreatePatient = async (e) => {
    ...
    if (!myInstLower) {
      setErr(
        "Your provider profile is missing an Institution. Please contact an admin."
      );
      return;
    }
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      if (filtered.length === 0 && term) {
        const mock = mockPatients.filter((p) =>
          p.name.toLowerCase().startsWith(term)
        );
        setResults(mock);
      } else {
  ...
  // ====== Select patient & subscribe to meds ======
  const openPatient = async (p) => {
    ...
    try {
      ...
      // leaving unsub as-is (simple demo)
    } catch (e) {
  ...
  const removeMedication = async (med) => {
    setMsg("");
    setErr("");
    if (!selectedPatient || !med?.id) return;
    ...
    try {
      ...
      setMsg("Medication removed.");
    } catch (e) {
      setErr(`Remove medication failed: ${e.message}`);
    }
  };
  ...
  const startEdit = () => {
    ...
    setEdit({
      ...
      allergies: Array.isArray(base.allergies)
        ? base.allergies.join(", ")
        : base.allergies || "",
    });
  };
  ...
  return (
    <div style={{ display: "flex" }}>
      ...
      <div style={pageWrap}>
        <div style={container}>
          {/* ===== Provider header (ONLY on overview) ===== */}
          {currentTab === "overview" && (
            <div
              style={{
                ...card,
            ...
            >
              <div>
                ...
                <div style={{ ...muted, fontSize: 14 }}>
                  Manage your patients, medications, and safety alerts.
                </div>
              </div>
              <button
                style={btnPrimary}
          ...
          {/* Banners */}
          {instLoading && (
            <div style={bannerWarn}>Loading your provider profile…</div>
          )}
          {!instLoading && !myInst && (
            <div style={bannerWarn}>
              Your provider profile has no <strong>Institution</strong>. New
              patients will be blocked until it’s set.
            </div>
          )}
                ...
                <div style={{ ...card, marginBottom: 16 }}>
                  ...
                  <div style={{ lineHeight: 1.9 }}>
                    ...
                    <div style={{ fontSize: 12, color: "#64748b" }}>
                      Share these with the patient for first login. In
                      production you’d likely force a password reset on first
                      login.
                    </div>
                  </div>
                  <div style={{ marginTop: 12, display: "flex", gap: 8 }}>
                    <button style={btnGhost} onClick={copyCreds}>
                      Copy
                    </button>
                  </div>
                </div>
                    ...
                    <div style={row2}>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          placeholder="(555) 555-5555 • 123 Main St"
                        />
                      </div>
                    </div>
                      ...
                      >
                        ...
                        <label style={{ fontSize: 12, color: "#0f172a" }}>
                          <input
                            ...
                            name="medSource"
                            value="all"
                            checked={medSource === "all"}
                            onChange={() => setMedSource("all")}
                            style={{ marginRight: 4 }}
                          />
                          All medications
                        </label>
                        <label style={{ fontSize: 12, color: "#0f172a" }}>
                          <input
                            ...
                            name="medSource"
                            value="otc"
                            checked={medSource === "otc"}
                            onChange={() => setMedSource("otc")}
                            style={{ marginRight: 4 }}
                          />
                          OTC only
                        </label>
                      ...
                      {medsDraft.map((m, i) => (
                        ...
                        >
                          ...
                          <select
                            ...
                            value={m.kind}
                            onChange={(e) =>
                              updateMed(i, "kind", e.target.value)
                            }
                          >
                          ...
                          <input
                            ...
                            value={m.notes}
                            onChange={(e) =>
                              updateMed(i, "notes", e.target.value)
                            }
                          />
                    ...
                    <div style={{ marginTop: 8 }}>
                      ...
                      <div style={{ marginTop: 12 }}>
                        ...
                        <input
                          ...
                          placeholder="Follow-up BP check"
                        />
                      </div>
                    </div>
              ...
              <div style={{ ...card, marginBottom: 16 }}>
                ...
                {results.length > 0 ? (
                  ...
                  <p style={{ ...muted, marginTop: 12 }}>
                    {myInstLower
                      ? "No results yet. Try a name, or leave blank and press Search to see recent."
                      : "Enter via a provider account with an Institution to search."}
                  </p>
                )}
              </div>
                    ...
                    <>
                      ...
                      >
                        ...
                        <div style={{ gridColumn: "1 / -1" }}>
                          ...
                          {selectedPatient.addr || "—"}
                        </div>
                        <div style={{ gridColumn: "1 / -1" }}>
                          <strong style={{ color: "#0f172a" }}>
                            Allergies:
                          </strong>{" "}
                          {Array.isArray(selectedPatientDoc?.allergies) &&
                          selectedPatientDoc.allergies.length
